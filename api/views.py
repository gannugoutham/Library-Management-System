    ...
    if session.get("email") is not None:    
        ...
        for p in products:
            try:
                ...
                status = "Request"
            product_list.append([p.product_name, p.category_name, p.quantity, status, p.id])
        print(product_list)
...
def status_change():
    ...
    values = Dashboard(id=id, email=email, product_name=product_name, category_name= product_category, status="Requested")
    db.session.add(values)
    db.session.commit()
...
def request_product():
    ...
    values = Dashboard(product_id=id, email=email, product_name=product_name, category_name= product_category, status="Requested")
    db.session.add(values)
    db.session.commit()
